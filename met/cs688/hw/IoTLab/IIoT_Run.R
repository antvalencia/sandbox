...
# Plot Results 
Range.to.Plot <- seq(
  from = round(dim(InputData)[1]*(1-Output$Percent.To.Test)),
  to = dim(InputData)[1]
  )
...
Title <- paste(
  ...
  paste0(
    "(Train End Date is ",
    Output$TimeStamp[Range.to.Plot[1]]
    ),
  ")"
  )
...
p <- plot_ly(
  ...
  x=~x,
  y=~y1,
  name = 'Actual Electric Demand (kW)',
  type = 'scatter',
  mode = 'lines'
)
p <- add_trace(
  p,
  y=~y2,
  name = 'RNN Forecasted Demand (kWh)',
  mode = 'lines',
  symbol = I(1),
  marker = list(size = 5)
  ) # add another line
